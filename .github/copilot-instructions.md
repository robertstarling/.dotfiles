...
1. **Setup scripts** (root `setup*.sh`) — Entry points that orchestrate installation. There are environment-specific variants:
   - `setup.sh` — Full setup (zsh-based environment with neovim, tmux, fzf, etc.)
...
3. **Config files** (`links/`) — Dotfile configs (tmux, nvim, zsh, git, etc.) that get symlinked into `$HOME` by `src/linkdotfiles/`.
...
- **Shell scripts use `#!/bin/bash`** and source the shared logger from `links/zsh/utils/output/log.sh`.
- **Git submodules** are used for zsh plugins (fzf-tab, fzf-git.sh) and tmux plugin manager (tpm).
